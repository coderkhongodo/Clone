...
"""
Demo script thử nghiệm Multilingual BERT cho clickbait detection
...
class MultilingualBertDataset(torch.utils.data.Dataset):
    """Dataset class cho Multilingual BERT"""
...
class MultilingualBertClickbaitClassifier:
    """Multilingual BERT classifier cho clickbait detection"""
    
    def __init__(self, model_name: str = 'bert-base-multilingual-cased', max_length: int = 512):
        self.model_name = model_name
        self.max_length = max_length
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.label_names = ['non-clickbait', 'clickbait']
        
        print(f"🚀 Khởi tạo Multilingual BERT Classifier")
    ...
    def create_datasets(self, data: Dict):
        ...
        train_dataset = MultilingualBertDataset(
            data['train']['texts'], 
            data['train']['labels'], 
            self.tokenizer, 
            self.max_length
        ...
        val_dataset = MultilingualBertDataset(
            data['val']['texts'], 
            data['val']['labels'], 
            self.tokenizer, 
            self.max_length
        ...
        test_dataset = MultilingualBertDataset(
            data['test']['texts'], 
            data['test']['labels'], 
            self.tokenizer, 
            self.max_length
    ...
    def train(self, train_dataset, val_dataset, output_dir: str = './multilingual_bert_results', 
    ...
    def evaluate(self, test_dataset, output_dir: str = './multilingual_bert_results'):
    ...
    def plot_confusion_matrix(self, true_labels, predictions, output_dir):
        ...
        plt.title('Confusion Matrix - Multilingual BERT Clickbait Detection\n' + 
    ...
    def predict_single(self, text: str, output_dir: str = './multilingual_bert_results') -> Dict:
    ...
    def generate_evaluation_report(self, results: Dict, output_dir: str):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("📊 MULTILINGUAL BERT CLICKBAIT DETECTION - EVALUATION REPORT\n")
            f.write("="*80 + "\n\n")
...
def demo_training(args):
    """Demo training Multilingual BERT"""
    print("="*60)
    print("🚀 DEMO TRAINING MULTILINGUAL BERT")
    print("="*60)
    
    # Initialize classifier
    classifier = MultilingualBertClickbaitClassifier(
...
def demo_interactive(args):
    ...
    print("="*60)
    print("🎮 DEMO INTERACTIVE - MULTILINGUAL BERT")
    print("="*60)
...
def main():
    parser = argparse.ArgumentParser(description='Multilingual BERT Clickbait Detection Demo')
    ...
    parser.add_argument('--output_dir', type=str, default='./multilingual_bert_results',
